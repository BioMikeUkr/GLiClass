...
class GLiClassUniEncoder(GLiClassBaseModel):
    ...
    def ForCausalLMLoss(
        ...
        **kwargs,
        ...
        loss = fixed_cross_entropy(logits, shift_labels, num_items_in_batch, ignore_index, reduction="sum",**kwargs)
        return loss
    ...
    def causal_forward(self, token_embeds, input_ids, attention_mask):
        ...
        loss = self.ForCausalLMLoss(
            vocab_size = self.config.decoder_config.vocab_size,
            shift_labels = labels,
            logits=outputs.logits,
            labels=None,
            num_items_in_batch= labels.size(0),
            ignore_index=-100
        )
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.config.layer_wise and labels is not None:
            ...
            # if labels is not None and self.config.decoder_config:
            if self.config.decoder_config:
                ...
                # loss = loss*2 + decoder_loss if decoder_loss is not None else loss
                # loss = (loss + decoder_loss) / 2 if decoder_loss is not None else loss
                # print(f"Decoder loss: {decoder_loss.item() if decoder_loss is not None else None}")
